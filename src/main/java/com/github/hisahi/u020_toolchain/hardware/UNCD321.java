...
import com.github.hisahi.u020_toolchain.cpu.instructions.Operations;
...
public class UNCD321 extends Hardware {
    ...
    public void hwi(UCPU16 cpu) {
        ...
        case 4:
            if (this.lem) {
                ...
                for (int i = 0; i < 128; ++i) {
                    cpu.getMemory().array()[(b + i) & 0xFFFF] = defaultfontlem[i];
                }
            } else {
                this.cpu.addCycles(1280);
                for (int i = 0; i < 1280; ++i) {
                    cpu.getMemory().array()[(b + i) & 0xFFFF] = defaultfont[i];
                }
            }
            break;
        case 5:
            this.cpu.addCycles(16);
            for (int i = 0; i < 16; ++i) {
                this.cpu.getMemory().write(b + i, defaultpal[i]);
            }
            break;
        case 6:
            this.mode = b & 3;
    ...
    public void reset() {
        ...
        this.font = Arrays.copyOf(this.defaultfont, this.defaultfont.length);
        this.palette = Arrays.copyOf(this.defaultpal, this.defaultpal.length);
    ...
    public void drawSprite(int[] memarr, int s) {
        ...
        int ax = Operations.signExtend16_32(memarr[(sp + 4) & 0xFFFF]);
        int ay = Operations.signExtend16_32(memarr[(sp + 5) & 0xFFFF]);
