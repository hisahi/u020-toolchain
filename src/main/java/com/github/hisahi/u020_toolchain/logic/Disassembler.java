...
public class Disassembler {
    ...
    private static AssemblyListing disassembleInstruction(int[] memory, int pos) {
        ...
        if (instr == null) {
            return new AssemblyListing(opos, copyOfRange_wraparound(memory, opos, pos), "??? illegal instruction!");
        }
        String finalstr;
        if (o == 0) {
        ...
        return new AssemblyListing(opos, copyOfRange_wraparound(memory, opos, pos), finalstr);
    }
    private static int[] copyOfRange_wraparound(int[] memory, int a, int b) {
        int l = (b - a);
        int[] res = new int[l];
        for (int i = 0; i < l; ++i) {
