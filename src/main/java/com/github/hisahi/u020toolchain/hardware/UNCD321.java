...
public class UNCD321 extends Hardware {
            ...
            case 1:
                if (this.lem) {
                    ...
                    for (int c = 0; c < 128; ++c) {
                        ...
                        if (b == 0) {
                            lo = DEFAULT_FONT_LEM[c * 2];
                            hi = DEFAULT_FONT_LEM[c * 2 + 1];
                        }
    ...
    private void drawCharacterRow(int[] memarr, int bp, int chr, boolean blink, int bgc, int fgc) {
        for (int x = 0; x < 8; ++x) {
            if (((chr & 0x8000) != 0) != blink) {
                if (fgc != 0) {
                    setCollision(memarr, bp + x, -1);
                    buffer[bp + x] = fgc;
                }
            } else {
                if (blink) {
    ...
    public void displayFrame(IPixelWriter ipw, EmulatorMain emu) {
        ...
        if (memsprite > 0) {
            ...
            for (int s = 15; s >= 0; --s) {
                if (memarr[(this.memsprite + 16 * s) & 0xFFFF] != 0) {
                    if ((memarr[(this.memsprite + 16 * s + 6) & 0xFFFF] & 0x20) != 0) {
            ...
            for (int row = 1; row < 191; row += 10) {
                for (int col = 0; col < 256; col += 8) {
                    ...
                    for (int y = 0; y < 10; y += 2) {
                        ...
                        chr = this.font[base];
                        base += 0x100;
                        drawCharacterRow(memarr, bp, chr, blink && cursoron, bgc, fgc);
                        drawCharacterRow(memarr, bp + 256, chr << 8, blink && cursoron, bgc, fgc);
                    }
                }
            }
        } else if (mode == 1) {
        ...
        ipw.write(0, 0, 256, 192, buffer, 256);
        if (this.vsyncint != 0) {
    ...
    private void setCollision(int[] memarr, int p, int d) {
        ...
        if (d > 0) {
            if (this.collisions[p] > 0) {
                memarr[base + 16 * this.collisions[p]] |= 1;
                memarr[base + 16 * d] |= 1;
            }
            if (this.collisions[p] < 0) {
                memarr[base + 16 * d] |= 2;
    ...
    private void drawSprite(int[] memarr, int s) {
        ...
        int[] sprpal = convertRange(memarr, sp + 8, 8);
        for (int oy = 0; oy < 16; ++oy) {
            ...
            for (int ox = 0; ox < 16; ++ox) {
                ...
                if (c != 0) {
                    int sc = sprpal[c];
                    buffer[bp] = sc;
                    setCollision(memarr, bp, s + 1);
                    if ((cf & 2) != 0) {
                        buffer[bp + 1] = sc;
                        setCollision(memarr, bp + 1, s + 1);
                    }
                    if ((cf & 1) != 0) {
                        buffer[bp + 256] = sc;
                        setCollision(memarr, bp + 256, s + 1);
                    }
                    if ((cf & 1) != 0 && (cf & 2) != 0) {
                        buffer[bp + 257] = sc;
                        setCollision(memarr, bp + 257, s + 1);
                    }
                }
            } 
            ++ptr;
    ...
    private int[] convertRange(int[] arr, int start, int len) {
        int[] res = new int[len];
        for (int i = 0; i < len; ++i) {
            res[i] = 0xff000000 | ((((arr[start + i] >> 8) & 15) * 17) << 16)  
                                | ((((arr[start + i] >> 4) & 15) * 17) << 8)  
                                |   ((arr[start + i]       & 15) * 17);
        }
        return res;
    }
